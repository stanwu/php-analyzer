...
def main(args):
    ...
    print_summary(f"[*] Found {len(custom_php_files)} custom PHP files (excluded {excluded_count} vendor files)", Colors.BLUE)
    ...
    for mode in modes_to_run:
        ...
        for file in custom_php_files:
            ...
            for finding in findings:
                if Severity.__args__.index(finding.severity) <= Severity.__args__.index(args.severity):
                    results[f"{mode}_scanner"].append(finding)
        print_summary(f"[*] Running {mode} scanner... done ({len(results[f'{mode}_scanner'])} findings)", Colors.GREEN, file=sys.stderr)
    ...
    if args.mode == "all" or args.mode == "deps":
        ...
        print_summary(f"[*] Running dependency scanner... done (graph: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges)", Colors.GREEN)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["security", "deps", "dead", "credential", "all"], default="all", help="Which scanners to run (default: all)")
    ...
    parser.add_argument("--severity", choices=Severity.__args__, default="HIGH", help="Minimum severity to report (default: HIGH)")
